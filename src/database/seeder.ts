...
// ── Grados ────────────────────────────────────────────────
interface GradoSeed { nombre: string; numero: number; }

const GRADOS: GradoSeed[] = [
  { nombre: 'Primer grado',    numero: 1  },
  { nombre: 'Segundo grado',   numero: 2  },
  { nombre: 'Tercer grado',    numero: 3  },
  { nombre: 'Cuarto grado',    numero: 4  },
  { nombre: 'Quinto grado',    numero: 5  },
  { nombre: 'Sexto grado',     numero: 6  },
  { nombre: 'Séptimo grado',   numero: 7  },
  { nombre: 'Octavo grado',    numero: 8  },
  { nombre: 'Noveno grado',    numero: 9  },
  { nombre: 'Décimo grado',    numero: 10 },
  { nombre: 'Undécimo grado',  numero: 11 },
];
...
async function main(): Promise<void> {
  ...
  for (const g of GRADOS) {
    await pool.query(
      `INSERT IGNORE INTO grado (nombre, numero, area) VALUES (?, ?, 'ciencias_sociales')`,
      [g.nombre, g.numero]
    );
  ...
  for (const dba of DBAS) {
    ...
    if (!gradoRows[0]) {
      console.warn(`  ⚠  DBA ${dba.codigo_men} - grado no encontrado`);
      continue;
    }

    await pool.query(
      `INSERT IGNORE INTO dba_catalogo
         (grado_id, codigo_men, enunciado_oficial, evidencias_aprendizaje)
       VALUES (?, ?, ?, ?)`,
      [gradoRows[0].id, dba.codigo_men, dba.enunciado_oficial, dba.evidencias_aprendizaje]
    );
  ...
  console.log('  GET /api/catalogo/dbas?grado_id=5\n');
}
